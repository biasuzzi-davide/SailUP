...
if (isset($_GET['id']) && trim($_GET['id']) !== '') {
    ...
    if (is_array($prod)) {
        ...
        $placeholders = array_merge($placeholders, [
            '[PROD_NAME]' => optionalValueAttr($prod['Nome_Prodotto'] ?? ''),
            ...
            '[PROD_DURATION]' => optionalValueAttr($prod['Durata_Ore'] ?? ''),
            '[PROD_DESC]' => htmlspecialchars($prod['Descrizione_Breve'] ?? ''),
            '[PROD_DESC_LONG]' => htmlspecialchars($prod['Descrizione'] ?? ''),
            '[PROD_PRICE]' => optionalValueAttr($prod['Prezzo_Base'] ?? ''),
            '[PROD_CAPACITY]' => optionalValueAttr($prod['Posti_Totali'] ?? ''),
            '[PROD_LENGTH]' => optionalValueAttr($prod['Lunghezza_Barca_Metri'] ?? ''),
            '[IMG_URL]' => optionalValueAttr($prod['URL_Media'] ?? ''),
            '[IMG_ALT]' => optionalValueAttr($prod['Testo_Alternativo'] ?? ''),
    ...
    if (!verifyCsrfToken($csrf)) {
        ...
        if (empty($errors)) {
            ...
            if ($ok) {
                ...
                $placeholders = array_merge($placeholders, [
                    '[PROD_NAME]' => optionalValueAttr($nome),
                    ...
                    '[PROD_DURATION]' => optionalValueAttr((string)$durata),
                    '[PROD_DESC]' => htmlspecialchars($descrBreve),
                    '[PROD_DESC_LONG]' => htmlspecialchars($descr),
                    '[PROD_PRICE]' => optionalValueAttr((string)$prezzo),
                    '[PROD_CAPACITY]' => optionalValueAttr((string)$posti),
                    '[PROD_LENGTH]' => optionalValueAttr((string)($lunghezza ?? '')),
                    '[IMG_URL]' => optionalValueAttr($finalUrl),
                    '[IMG_ALT]' => optionalValueAttr($altImg),
        ...
        $placeholders = array_merge($placeholders, [
            ...
            '[PROD_CAPACITY]' => optionalValueAttr((string)$posti),
            '[PROD_LENGTH]' => optionalValueAttr((string)($lunghezza ?? '')),
            '[IMG_URL]' => optionalValueAttr($urlImg),
            '[IMG_ALT]' => optionalValueAttr($altImg),
            '[CHECK_PATENTE]' => $richiedePatente ? 'checked' : '',
            '[CHECK_ACCESS]' => $accessibile ? 'checked' : '',
            '[CHECK_STATUS_AVAILABLE]' => $status === 'available' ? 'checked' : '',
