...
if (is_array($pren) && !empty($pren)) {
    foreach ($pren as $p) {
        ...
        $rows .= '<tr>'
            ...
            . '<a href="dettaglio_prenotazione.php?id=' . htmlspecialchars($p['IDPrenotazione']) . '" class="btn-icon" aria-label="Vedi dettagli prenotazione ' . htmlspecialchars($p['IDPrenotazione']) . '">👁️</a>'
            . '<form method="post" class="inline-form">'
            . '<input type="hidden" name="csrf_token" value="' . $csrfToken . '">'
            . '<input type="hidden" name="id_prenotazione" value="' . htmlspecialchars($p['IDPrenotazione']) . '">'
            . '<input type="hidden" name="action" value="confirm">'
            . '<button type="submit" class="btn-icon" aria-label="Conferma prenotazione ' . htmlspecialchars($p['IDPrenotazione']) . '">✅</button>'
            . '</form>'
            . '<form method="post" class="inline-form" onsubmit="return confirm(\'Cancellare questa prenotazione?\');">'
            . '<input type="hidden" name="csrf_token" value="' . $csrfToken . '">'
            . '<input type="hidden" name="id_prenotazione" value="' . htmlspecialchars($p['IDPrenotazione']) . '">'
            . '<input type="hidden" name="action" value="cancel">'
            . '<button type="submit" class="btn-icon danger" aria-label="Cancella prenotazione ' . htmlspecialchars($p['IDPrenotazione']) . '">🗑️</button>'
            . '</form>'
            . '</td>'
            . '</tr>';
    }
